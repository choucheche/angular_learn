...
gulp.task('script',function(){
  gulp.src([app.srcPath+'/js/**/*.js',app.no_srcPath+'/js/**/*.min.js',app.no_srcPath+'/js/**/*0.js'])
    .pipe(plumber({errorHandler: notify.onError('Error: <%= error.message %>')}))
    //当编译时出现语法错误或者其他异常,出现异常并不终止watch事件（gulp-plumber），并提示我们出现了错误（gulp-notify）。
    .pipe(jshint()) //检查JS错误
...
//编译less转为css
  gulp.src([app.srcPath+'/less/**/*',app.no_srcPath+'/less/**/*0.less'])
    //.pipe(plumber({errorHandler: notify.onError('Error: <%= error.message %>')}))
    .pipe(plumber())
    //当编译时出现语法错误或者其他异常,出现异常并不终止watch事件（gulp-plumber），并提示我们出现了错误（gulp-notify）。
